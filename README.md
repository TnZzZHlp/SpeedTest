...
多线程网络下载/上传测速工具，交互终端默认使用 ratatui TUI，支持纯文本命令行模式。
...
- 深色面板、彩色实时下载/上传和平均速度卡片，以及最近 120 次采样的下载趋势图。
- 显示累计下载量、上传量、运行时间、测速分组、并发上限、重试次数和当前下载地址。
...
测速持续下载直至退出，会消耗网络流量；内置地址模式同时向 Apple 的 `/api/v1/gm/slurp` 持续发送二进制 POST 测量上传速度。默认按 HTTP 响应时间选择可用服务器，并非逐个比较下载吞吐量；指定 `--url` 后只使用该地址且不启用 Apple 上传。HTTP 错误响应不计入下载量。
